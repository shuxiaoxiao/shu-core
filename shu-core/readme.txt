...
更新时间：2016-12-26
增加内容：
	（1）增加ZipUtil类——打包下载，解压等方法
	

===========版本：1.0.2==============
更新时间：2016-12-12
增加内容：
	（1）重新写excel工具类，使用父类来接收，不用写2遍，且在导出加上了sheet分页，默认每页20条
	（2）	MathUtil 加上新的随机生成方法, SystemUtil里面对其进行了组合
	

===========版本：1.0.1==============
更新时间：2016-10-10
增加内容：
	（1）解决freemarker模板生成有多个库中表名是一样时的问题：model对象和mapping文件里面就有多个库该表的所有字段
	（2）分页查询（select2PageBean）、条件查询（selectAllByCondition）分开因为查询条件会不同，展示的字段也会有差异
